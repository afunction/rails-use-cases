# Summary

* [Mixin and Concerns](mixin_and_concerns/README.md)
   * [Include & Extend](mixin_and_concerns/usage.md)
   * [Concerns](mixin_and_concerns/concerns.md)
   * [Use Cases](mixin_and_concerns/use_cases.md)
       * [DRY model](mixin_and_concerns/dry-your-model.md)
       * [擴充已存在物件](mixin_and_concerns/extend-exists-classes.md)
       * [留 options 給使用的 Class ](mixin_and_concerns/setup-options-in-objects.md)
       * [搭配 meta-programming](mixin_and_concerns/meta-programming.md)
   * [Tips](mixin_and_concerns/tips.md)
* [Refactor Patterns](refactor_patterns/README.md)
   * [Form Objects](refactor_patterns/form_objects.md)
       * [深入 Form helper](refactor_patterns/details_form_helper.md)
       * [實作 Form Object](refactor_patterns/implement_form_objects.md)
   * [Service Objects](refactor_patterns/service_objects.md)

